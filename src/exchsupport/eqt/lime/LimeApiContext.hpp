#ifndef SRC_EQT_LIMEAPICONTEXT_HPP_
#define SRC_EQT_LIMEAPICONTEXT_HPP_ 
...
class LimeApiListener : public Listener {
public:
	LimeApiListener(LimeApiContext& ctx, KTN::EQT::EqtGT4& owner) : apiCtx(ctx), apiOwner(owner) {}

private:
    void onOrderAccept(OrderId orderId, OrderId limeOrderId, const OrderAckAttributes& attributes, uint64_t eventId);
    void onOrderFill(OrderId orderId, const FillInfo& fillInfo, uint64_t eventId);
    void onUSOptionsOrderFill(OrderId orderId, const USOptionsFillInfo& fillInfo, uint64_t eventId);
    ...
    void onOrderReplace(OrderId orderId, OrderId replaceOrderId, OrderId limeReplaceOrderId, const OrderAckAttributes& attributes, uint64_t eventId);
    ...
    void onManualOrder(OrderId orderId, const ManualOrderInfo& info, uint64_t eventId);
    void onManualUSOptionsOrder(OrderId orderId, const ManualUSOptionsOrderInfo& info, uint64_t eventId);
...
private:
    LimeApiContext& apiCtx;
    KTN::EQT::EqtGT4& apiOwner;
};
...
public:
    ...
    void updateEventId(uint64_t eventId);
    void dumpStats();
    void clearStats();
...
private:
    ...
    LimeApiListener listener;
    KTN::EQT::EqtGT4 &owner;
    bool markForDeletion;
};


#endif /* SRC_EQT_LIMEAPICONTEXT_HPP_ */
