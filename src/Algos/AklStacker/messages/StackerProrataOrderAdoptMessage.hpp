...
struct StackerProrataOrderAdoptMessage
{
	static constexpr MessageType type = MessageType::STACKER_PRORATA_ORDER_ADOPT;
	std::string id;
	uint64_t oid;
	//std::string oid;
	...
	static StackerProrataOrderAdoptMessage Deserialize(const rapidjson::Document& document)
	{
		StackerProrataOrderAdoptMessage message;
	...
	static StackerProrataOrderAdoptMessage DeserializeFromString(const std::string& json)
	{
		rapidjson::Document document;
		...
		{
			return StackerProrataOrderAdoptMessage();
		}
