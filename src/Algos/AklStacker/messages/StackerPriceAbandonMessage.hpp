...
struct StackerPriceAbandonMessage
{
	struct level
	{
		Price price;
	...
	static constexpr MessageType type = MessageType::STACKER_PRICE_ABANDON;
	std::string id;
	...
	std::string SerializeToString() const
	{
		...
		static StackerPriceAbandonMessage Deserialize(const rapidjson::Document& document)
		{
			StackerPriceAbandonMessage message;
		...
		static StackerPriceAbandonMessage DeserializeFromString(const std::string& json)
		{
			rapidjson::Document document;
			...
			if (document.HasParseError())
			{
				...
				return StackerPriceAbandonMessage();
			}
