...
MS2Params MS2Params::parse(const std::string &json_str)
{
	MS2Params cp;
	cp.ValidJson = false;
	...
	for (Value::ConstValueIterator itr = d.Begin(); itr != d.End(); ++itr)
	{
		...
		cp.TestMode = (*itr)["TestMode"].GetBool();

		cp.MaxPosition = (*itr)["MaxPosition"].GetDouble();
		cp.TotalQty = (*itr)["TotalQty"].GetDouble();

		cp.MinLeanQty = (*itr)["MinLeanQty"].GetInt();
