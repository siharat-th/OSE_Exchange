//============================================================================
// Name        	: SRC_ALGOPARAMS_ICEBERGPARAMS_HPP_.cpp
// Author      	: Sam Gaer
// Version     	:
...
#include <AlgoParams/IcebergParams.hpp>


IcebergParams IcebergParams::parse(const std::string &json_str)
{
	IcebergParams cp;
	cp.ValidJson = false;
	...
	for (Value::ConstValueIterator itr = d.Begin(); itr != d.End(); ++itr)
	{
		...
		//cp.TotalQty = (*itr)["TotalQty"].GetInt();
